...
|---|---|---|---|
| 인증·학번 | 아이디(가상 이메일)+8자리 학번 형식검증 | @chungbuk.ac.kr 이메일 인증+10자리 학번 | 학교 이메일 인증 채택 + 가입 시 학과·10자리 학번 입력. 학번은 형식·입학연도 검증만 수행 |
...
| 학번 | text | 10자리 숫자, 입학연도 검증 | "학번은 10자리 숫자여야 합니다." |
...
```
형식: YYYY000000 (앞 4자리: 입학연도, 뒤 6자리: 일련번호)
 - 입학연도: 1990 ~ 현재연도 범위
 - 숫자 외 입력 불가 (pattern="[0-9]{10}", maxLength=10)
...
CREATE TABLE profiles (
  ...
  full_name    TEXT,
  student_id   CHAR(10) NOT NULL,            -- 10자리 학번
  department   TEXT,
